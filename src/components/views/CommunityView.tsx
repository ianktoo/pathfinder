            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-10 pb-24">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
                        {['All', 'Romantic', 'Adventure', 'Chill', 'Party'].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${filter === f ? 'bg-stone-900 text-white dark:bg-white dark:text-black' : 'bg-stone-100 text-stone-600 dark:bg-neutral-800 dark:text-stone-400'}`}
                            >
                                {f}
                            </button>
                        ))}
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filtered.map(it => (
                            <div key={it.id} className="bg-white dark:bg-neutral-900 rounded-3xl border border-stone-200 dark:border-neutral-800 overflow-hidden shadow-sm hover:shadow-lg transition-all flex flex-col">
                                <div className="h-48 relative bg-stone-200">
                                    <img src={it.items[0]?.imageUrl || `https://source.unsplash.com/random/800x400?${it.mood}`} className="w-full h-full object-cover" alt="Cover" />
                                    <div className="absolute top-4 left-4">
                                        <span className="bg-white/90 backdrop-blur text-stone-900 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                                            {it.mood}
                                        </span>
                                    </div>
                                    <div className="absolute bottom-4 left-4 flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full bg-orange-500 flex items-center justify-center text-white font-bold text-xs ring-2 ring-white">
                                            {it.author?.[0] || 'A'}
                                        </div>
                                ...
                                <div className="p-6 flex-1 flex flex-col">
                                    <h3 className="text-xl font-black text-stone-900 dark:text-white mb-2">{it.title}</h3>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-stone-100 dark:border-neutral-800 flex justify-between items-center">
                                        <Button variant="outline" size="sm" icon={Heart} className="rounded-full w-10 h-10 p-0" />
                                        <Button onClick={() => onClone(it)} size="sm" icon={Download} className="rounded-full px-6">
                                            Clone to Library
                                        </Button>
                                    </div>
                                </div>
